...
class ResultMapping(OrderedDict):
    """Ordered, arbitrarily nested formula results.

    ``map`` applies a callable to every leaf while preserving the mapping
    structure. ``flatten`` returns another ``ResultMapping`` whose keys are
    tuples containing each leaf's full path.
    """
...
class _StructuredRuntime:
    ...
    def _structured(self, result: Any) -> Any:
    ...
    def run_batch(self, *args: Any, **kwargs: Any) -> Any:
        return self._structured(self._runtime.run_batch(*args, **kwargs))

    def run_batch_from_mapping(self, *args: Any, **kwargs: Any) -> Any:
        return self._structured(self._runtime.run_batch_from_mapping(*args, **kwargs))
...
def support_formula_mappings(compile_function):
    """Add nested mappings through the existing fused multi-output compiler."""
