...
def test_result_mapping_map_and_flatten_preserve_nested_order():
    values = ResultMapping(
        (("signals", ResultMapping((("fast", 1), ("slow", 2)))), ("risk", 3))
    )
    mapped = values.map(lambda value: value * 10)
    assert isinstance(mapped, ResultMapping)
    assert isinstance(mapped["signals"], ResultMapping)
...
def test_formula_mapping_adapter_compiles_flat_once_and_reconstructs_load():
    ...
    runtime = compile_formula(
        OrderedDict(
            (
                ("signals", OrderedDict((("fast", "f5"), ("slow", "f20")))),
                ("risk", "f60"),
            )
        )
    )
