...
def test_expr_pipe_supports_positional_and_keyword_injection():
    ...
    keyword = expression.pipe(
        (lambda *, value, amount: dsl.add(value, amount), "value"), amount=2
    )
...
def test_lazily_registered_dsl_function_is_method_chainable():
    ...
    dsl._register_dsl_method(name, registered)
    try:
        assert str(getattr(expression, name)(amount=3)) == str(
            registered(expression, amount=3)
        )
    finally:
        dsl._DSL_METHOD_REGISTRY.pop(name, None)
