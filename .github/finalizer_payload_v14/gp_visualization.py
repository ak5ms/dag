...
def gp_graph_data(pset: Any) -> tuple[tuple[GPGraphNode, ...], tuple[GPGraphEdge, ...]]:
    ...
    primitives: dict[tuple[Any, ...], Any] = {}
    terminals: dict[tuple[Any, ...], Any] = {}
    ...
    for primitive in sorted(primitives.values(), key=lambda item: str(item.name).lower()):
        ...
        if section:
            detail += f"\nSection: {section}"
        node_id = f"operator:{name}:{len(nodes)}"
        nodes.append(
            GPGraphNode(
                node_id,
                name,
...
def _positions(nodes: tuple[GPGraphNode, ...]) -> dict[str, tuple[float, float]]:
    ...
    for kind in ("terminal", "type", "operator"):
        values = sorted(
            (node for node in nodes if node.kind == kind),
    ...
    def write_html(
        ...
        *,
        ...
        destination.write_text(
            self.to_html(include_plotlyjs=include_plotlyjs), encoding="utf-8"
        )
        return destination
