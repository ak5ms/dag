...
def _legacy_batch(expr, values):
    runtime = engine.compile_formula(expr, cpp=False)
    inputs = (jnp.asarray(values, dtype=jnp.float64),)
    state, output, _ = engine._jit_batch_from_initial_state(runtime, inputs)
...
def test_named_outputs_share_one_dag_and_return_dict(monkeypatch):
    monkeypatch.setattr(engine, "_BATCH_CHUNK_SIZE", 96)
    ...
    separate_node_count = sum(
        len(engine.compile_formula(expr, cpp=False).program.nodes)
...
def test_planned_stateful_runtime_does_not_regress_materially(monkeypatch):
    monkeypatch.setattr(engine, "_BATCH_CHUNK_SIZE", 4096)
    ...
    legacy_runtime = engine.compile_formula(expr, cpp=False)
    planned_runtime = compile_formula(expr, cpp=False)
    inputs = (values,)

    def legacy_call():
        result = engine._jit_batch_from_initial_state(legacy_runtime, inputs)
