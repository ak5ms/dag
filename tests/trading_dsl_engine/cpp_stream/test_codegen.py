...
def test_codegen_embeds_typed_row_widths_and_promotes_only_at_operation():
    ...
    assert "stackdsl::BinaryExprSrc<" in source
    assert "stackdsl::AddOp" in source
    assert "stackdsl::BinaryNode<9," not in source
...
def test_generated_runtime_has_one_row_pass_and_cse_for_repeated_cat_branches():
    ...
    assert [stage.kind for stage in plan.stages] == ["cat"]
    assert plan.scratch_slots == 0
    assert source.count("stackdsl::BinaryNode<5,") == 0
    assert "stackdsl::BinaryExprSrc<" in source
...
def test_complex_stateless_stages_are_adjacent_inside_one_row_loop():
    ...
    assert source.count("stackdsl::BinaryNode<9,") == 0
    assert source.count("stackdsl::BinaryExprSrc<") == 3
...
def test_composed_ewm_co_moments_share_canonical_state_updates():
    formula = (
        "cat(ewm_co_skewness(x,y,span=8),"
        "ewm_co_kurtosis(x,y,span=8))"
    )
    program = compile_ir(formula)
    plan = lower_program(program, n_instruments=9)
    source = render_translation_unit(
        plan, n_instruments=9, prefetch_rows=16
    ).text

    assert [stage.kind for stage in plan.stages] == ["ewm_bundle", "cat"]
    assert len(plan.stages[0].bundle_outs) == 8
    assert plan.scratch_slots == 8
    assert source.count("stackdsl::EwmBundleNode<") == 1
    assert "stackdsl::EwmStatsNode<" not in source


def test_state_bundling_respects_intervening_physical_consumers():
    safe = lower_program(
        compile_ir("cat(ewm(x,3)+1,ewm(y,3))"),
        n_instruments=9,
    )
    unsafe = lower_program(
        compile_ir("cat(shift(ewm(x,3),1),ewm(y,3))"),
        n_instruments=9,
    )

    assert [stage.kind for stage in safe.stages] == ["ewm_bundle", "cat"]
    assert [stage.kind for stage in unsafe.stages] == ["ewm", "shift", "ewm", "cat"]


def test_literal_small_powers_lower_to_inline_multiplication():
    source = _render("x**2 + x**3 + x**4", n=9)
    assert "stackdsl::Pow2Op" in source
    assert "stackdsl::Pow3Op" in source
    assert "stackdsl::Pow4Op" in source
    assert "stackdsl::PowOp" not in source


def test_vector_moments_share_one_reduction_traversal():
    program = compile_ir("cat(vec_skewness(x),vec_kurtosis(x))")
    plan = lower_program(program, n_instruments=9)
    source = render_translation_unit(
        plan, n_instruments=9, prefetch_rows=16
    ).text

    assert [stage.kind for stage in plan.stages] == ["reduce_bundle", "cat"]
    assert len(plan.stages[0].bundle_outs) == 4
    assert source.count("stackdsl::ReductionBundleNode<") == 1
    assert source.count("stackdsl::FusedStageNode<") == 1


def test_sibling_ridge_projections_share_one_state_and_solve():
    model = "Ridge(1,x,y=y,hl=4,lambda_=0.2)"
    program = compile_ir(
        f"cat(get_sse({model}),get_r2({model}),get_effective_n({model}))"
    )
    plan = lower_program(program, n_instruments=9)
    source = render_translation_unit(
        plan, n_instruments=9, prefetch_rows=16
    ...
    assert [stage.kind for stage in plan.stages] == ["ridge_bundle", "cat"]
    assert len(plan.stages[0].bundle_outs) == 3
    assert source.count("stackdsl::RidgeBundleNode<") == 1
    assert source.count("stackdsl::FusedStageNode<") == 1
    assert "stackdsl::RidgeNode<" not in source
