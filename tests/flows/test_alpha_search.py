...
def test_default_pnl_and_ridge_pool_build_expected_formula_shapes():
    ...
    assert pnl.fn == "mul"
    pooled = ridge_pool_alpha_pnl(alpha, [var("alpha0")], roll_rets=var("roll_rets"), hs=var("hs"), is_tradable=var("is_tradable"), hl=1440)
...
def test_make_alpha_pset_uses_standard_deap_typed_groups():
    ...
    assert any(p.name == "ewm" and p.args == [Expr, PositiveScalar] for p in pset.primitives[Expr])
    assert any(p.name == "shift" and p.args == [Expr, PositiveIntScalar] for p in pset.primitives[Expr])
    assert any(p.name == "positive" and p.args == [Expr] for p in pset.primitives[PositiveScalar])
    assert any(p.name == "positive_int" and p.args == [Expr] for p in pset.primitives[PositiveIntScalar])
...
def test_dynamic_parameter_subtrees_compile_to_normal_dsl_expressions():
    ...
    expr = individual_to_expr(gp.PrimitiveTree.from_string("ewm(x, positive(xs_rank(adaptive)))", pset), pset)
    assert isinstance(expr, Call) and expr.fn == "ewm"
    assert "xs_rank" in repr(expr)
    expr = individual_to_expr(gp.PrimitiveTree.from_string("shift(x, positive_int(abs(adaptive)))", pset), pset)
    assert isinstance(expr, Call) and expr.fn == "shift"
    assert "floor" in repr(expr)
...
def test_same_type_family_uses_multi_type_intersection_algorithm(monkeypatch):
    ...
    monkeypatch.setattr(alpha_search, "analyze_formula_metadata", shared)
    pset = make_alpha_pset(["price", "volume"], operators=[OperatorSpec("add", alpha_search.add, 2, "same_type")])
    expr = individual_to_expr(gp.PrimitiveTree.from_string("add(price, volume)", pset), pset)
    assert expr.fn == "add"

    def disjoint(expr, config):
        return FakeMeta({"price"} if "price" in repr(expr) else {"volume"})

    monkeypatch.setattr(alpha_search, "analyze_formula_metadata", disjoint)
...
def test_depth_three_generation_example_filters_dimensionless_not_in_pool():
    ...
    def valid(expr):
        ...
        return repr(expr) not in pool_keys and "dimensionless" in meta.get_types() and isfinite(value_range.lower) and isfinite(value_range.upper)

    out = search_formulas(pset, lambda candidate, pool_: 1.0, max_depth=3, initial_pool=pool, filters=[valid], additive=lambda candidate, pool_, fitness: fitness > 0.0 and len(pool_) < 3, population_size=32, generations_per_depth=2, seed=11)
    assert out
...
def test_individual_to_expr_and_search_formulas_use_standard_deap_individuals():
    ...
    expr = individual_to_expr(gp.PrimitiveTree.from_string("ewm(x, hl_0)", pset), pset)
    assert isinstance(expr, Call) and expr.fn == "ewm"
    accepted_pool_sizes = []
    ...
    out = search_formulas(pset, objective, max_depth=2, filters=[lambda candidate: True], additive=lambda candidate, pool, fitness: fitness > 0.0 and len(pool) < 2, population_size=12, generations_per_depth=1, seed=7)
    assert out
    assert len(out) <= 2
