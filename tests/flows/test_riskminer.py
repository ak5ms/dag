...
from flows.riskminer.semantics import DEFAULT_TYPE_RELATIONS, compatible_additive, numeric_ratio
...
def test_market_semantics_and_generic_type_relations() -> None:
    ...
    price_sum = compatible_additive(fields["ap0"], fields["bp0"], DEFAULT_TYPE_RELATIONS)
    quantity_sum = compatible_additive(fields["av0"], fields["volume"], DEFAULT_TYPE_RELATIONS)
    invalid = compatible_additive(fields["close"], fields["volume"], DEFAULT_TYPE_RELATIONS)
    price_ratio = numeric_ratio(fields["ap0"], fields["bp0"], DEFAULT_TYPE_RELATIONS)
...
def test_typed_rpn_builds_valid_formula_and_rejects_bad_addition() -> None:
    ...
    for token in ("close", "volume", "add"):
        bad = environment.step(bad, environment.token_by_name[token].token_id)
...
def test_rpn_static_temporal_parameter_is_cpp_stream_compatible() -> None:
    environment = RPNEnvironment(max_depth=8, max_tokens=16)
    state = environment.parse(["close", "literal:20", "ewm", "xs_rank", "END"])
    assert state.complete
...
def test_operator_inventory_exposes_direct_and_structured_groups() -> None:
    ...
    assert {"add", "div", "ewm", "rolling_mean", "xs_rank"}.issubset(inventory["searchable"])
    assert {"cat", "einsum", "Ridge"}.issubset(inventory["structured"])


def test_cpp_stream_candidate_batch_matches_numpy_reference(tmp_path: Path) -> None:
    ...
    sources = {"soft_side_wavg": signal, "roll_rets": returns}
    candidates = [var("soft_side_wavg"), var("soft_side_wavg") * 2.0]
    evaluator = CppStreamCandidateEvaluator(sources, n_instruments=instruments, work_dir=tmp_path)
    actual = evaluator.score_batch(candidates)
    expected = [_score_reference(signal, returns), _score_reference(signal * 2.0, returns)]
...
class _FakeEvaluator:
    def score_batch(self, candidates):
        return [2.0 if "soft_side_wavg" in repr(candidate) else 0.1 for candidate in candidates]


def test_mcts_is_deterministic_and_discovers_finite_family() -> None:
    config = MCTSConfig(simulations=16, rollouts_per_expansion=3, selection_batch_size=4, archive_size=20, seed=11)
    first = RiskMinerMCTS(RPNEnvironment(max_depth=4, max_tokens=12), _FakeEvaluator(), config=config).search()
    second = RiskMinerMCTS(RPNEnvironment(max_depth=4, max_tokens=12), _FakeEvaluator(), config=config).search()
    assert first.candidates
    assert first.simulations == 16
    assert first.rollout_proposals >= 16
    assert [item.canonical_key for item in first.candidates] == [item.canonical_key for item in second.candidates]
...
def test_cpp_stream_ridge_pool_runs_final_only(tmp_path: Path) -> None:
    ...
    alphas = (var("soft_side_wavg"), (var("ap0") - var("bp0")) / var("close"))
    evaluator = CppStreamRidgePoolEvaluator(sources, n_instruments=instruments, work_dir=tmp_path, ridge_halflife=16.0)
