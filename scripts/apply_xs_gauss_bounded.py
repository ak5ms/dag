...
path = Path("src/trading_dsl_engine/cpp_stream/cpp/stackdsl/ops/naryop.hpp")
text = path.read_text()
...
for index, (old, new) in enumerate(replacements, start=1):
    ...
    if matches != 1:
        raise SystemExit(
            f"replacement {index} expected exactly one match, got {matches}"
        )
    text = text.replace(old, new, 1)

path.write_text(text)

python_path = Path("src/trading_dsl_engine/cpp_stream/python/xs_gauss.py")
python_text = python_path.read_text().replace(
