...
- `session_rbf_basis(ev_ts, session_start, session_end[, is_tradable], n_basis)` builds a non-periodic normalized radial basis from epoch timestamps and explicit session bounds, optionally masking non-tradable bars; `future_session_rbf_basis_sum(ev_ts, session_start, session_end[, next_session_start, next_session_end], n_basis, n_steps)` emits suffix sums of that basis over the remaining discrete session grid without materializing a `(n_instruments, n_basis, n_steps)` horizon cube.
