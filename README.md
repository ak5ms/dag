...
- `RBF_basis(x, n_basis)` builds a non-periodic normalized radial basis on an explicit `[0, 1]` phase; `RBF_basis(ev_ts, session_start, session_end[, is_tradable], n_basis)` derives that phase from epoch timestamps and explicit session bounds, optionally masking non-tradable bars. `future_RBF_basis_sum(...)` mirrors both forms and emits suffix sums over the remaining discrete phase/session grid without materializing a `(n_instruments, n_basis, n_steps)` horizon cube.
