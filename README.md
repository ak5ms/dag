...
- `BasisMean(features, y, weights, hl)` estimates feature-conditional means without a matrix solve; `InstrumentBasisMean(features, y, weights, hl)` keeps independent per-instrument basis means and emits an `(n_instruments, n_features)` beta matrix for high-throughput profile modeling.
- `rbf_basis(x, n_basis)` emits a non-periodic normalized radial basis matrix on `[0, 1]`; `session_rbf_basis(ev_ts, session_start, session_end[, is_tradable], n_basis)` builds the same basis from epoch timestamps and explicit session bounds, optionally masking non-tradable bars; `future_rbf_basis_sum(x, n_basis, n_steps)` and `future_session_rbf_basis_sum(ev_ts, session_start, session_end[, next_session_start, next_session_end], n_basis, n_steps)` emit suffix sums of that basis over the remaining discrete grid without materializing a `(n_instruments, n_basis, n_steps)` horizon cube.
...
The JAX backend accepts the same string formulas and Python-composed `Expr` trees as the Numba backend, including infix math/comparison operators and grouped-expression sugar such as `lhs.groupby((...)).apply(...)`. It stores formula structure in per-operator Equinox modules and wraps both the single-tick state transition and the batch `lax.scan` path with `eqx.filter_jit`, keeping the per-timestep hot paths compiled. It covers the scalar/vector/matrix stateless operators, `ewm`, `cumsum`, `segmented_cumsum`, `shift`, `rolling_quantile`, `xs_rank`, `outer`, `bspline`, `rbf_basis`, `session_rbf_basis`, `future_rbf_basis_sum`, `future_session_rbf_basis_sum`, `col`, `mean`, canonical tuple-key groupby `groupby((...), lhs, op_using_self_)` (including optional `univ(...)` inside the key tuple), and model projections via `get_beta(Ridge(...))`/`get_preds(Ridge(...))` plus `BasisMean(...)`/`InstrumentBasisMean(...)` for low-cost basis-conditioned volume/profile estimates.
