...
- **No interpreter hot loop**: `jax_flat` live ticks and batch operators execute through JAX JIT, using sequential scans only where the recurrence cannot be lowered to a parallel prefix.
...
For device-resident in-memory inputs, `jax_flat.run_batch(...)` uses one batch-wide XLA graph when every operator declares `batch_parallel=True`. Operators with a sequential batch recurrence use bounded compiled chunks instead, which controls working-set size for models such as `Ridge`; memmap inputs, disk caches, explicit `out_path`, and streaming time reductions also retain bounded host-side chunking. When `JAX_NUM_CPU_DEVICES` configures multiple logical CPU devices, a wholly stateless formula is time-sharded once around the complete DAG so operators need no individual `pmap` settings and XLA can still fuse within each shard. EWM lowers every `ignore_na`/`adjust` combination, dynamic span, and `min_periods` to shared affine or segmented associative prefixes while preserving live state at batch boundaries. `cumsum`, static-limit `ffill`, and `roll_mean` use the same prefix layer; arbitrary rolling callables and dynamic-limit `ffill` retain sequential scans. Associative prefixes use logical CPU devices while their operand footprint is within the scheduler-wide 128 MiB safety budget, configurable with `TRADING_DSL_JAX_FLAT_CPU_PREFIX_SHARD_MAX_BYTES`. `xs_rank` selects a fused pairwise comparison/reduction kernel for small cross-sections and the sort/search kernel for wider ones, avoiding large numbers of tiny serial sorts without changing tie or NaN semantics.
