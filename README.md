...
The native batch helper accepts `workers=None` (all available CPU cores) or a positive worker count on `CppFlatRuntime.run_batch(...)` and `run_batch_into(...)`. With multiple workers, a persistent C++ worker team schedules ready DAG nodes as soon as their dependencies complete, preserves the serial timestep boundary required by streaming state, and writes disjoint node/output buffers without holding the GIL. `workers=1` selects the serial baseline.
