...
- Keep basis-profile APIs unified: `RBF_basis(...)` and `future_RBF_basis_sum(...)` cover both explicit phase inputs and session-clock inputs; do not reintroduce separate phase-only/session-only public basis operators without a strong reason.
