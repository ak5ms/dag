...
- Treat tick diagnostics and batch execution as distinct lowering products: a compact tick JAXPR/HLO is not evidence that batch avoids time-axis intermediates. Batch executors must release each sequence after its final static DAG consumer, except terminal outputs/cache values. Evaluate batch lowering changes with the opt-in 1M x 9 operator-wise-versus-tick-scan benchmark, including dependent stateful and cross-sectional formulas. Prefer compositional, region/cost-based scheduling over an operator-name-specific batch branch; see `docs/jax_flat_batch_lowering_1m_x_9.md`.
