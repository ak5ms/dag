...
- Native scalar DAG fusion must be metadata-driven and generic across heterogeneous/nested `cat(...)` branches; keep any specialized family kernel only when benchmarks show it beats the generic transition without changing semantics.
