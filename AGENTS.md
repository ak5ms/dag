...
- For the JAX backend, keep live tick and batch hot paths under JAX JIT (`eqx.filter_jit`/`jax.jit`), and prefer functional PyTree state over Python mutation inside compiled execution. Use `lax.scan` only for genuinely sequential recurrences; associative state transitions should expose a batch-specific parallel prefix that returns exactly the same boundary state as repeated ticks. JAX-flat-only lag-history operators such as `buffer(shift(...), min_lag, max_lag)` should expose ordered ring state through compositional JAX ops, not Numba kernels. User-supplied `jax_flat.stateless(...)` callables must stay stateless and execute through the same generic compiled `NaryOp` tick/`vmap` batch path. When adding a JAX-flat operator, implement `scan_batch` alongside `init_state` and `tick` rather than relying on the default per-tick scan.
...
- Active JAX-flat associative-prefix and logical-CPU helpers: `src/trading_dsl_engine/jax_flat/parallel.py`
...
- Keep device-resident in-memory JAX formulas made entirely of time-parallel operators in one batch-wide graph so XLA can assign work across time. Operators opt into this generic scheduler path with the documented `batch_parallel` contract; the safe default is bounded compiled chunks for sequential recurrences. Do not add operator-type branches to the engine.
- If multiple logical CPU devices are configured, shard a wholly stateless formula around the complete DAG rather than adding `pmap` flags or thresholds to individual operators; exclude cross-time reductions and cache callbacks from this path.
- Express associative recurrences through the shared `associative_prefix(...)` helper so logical-device sharding and its scheduler-wide memory budget remain outside individual operator implementations.
- Keep all EWM parameter modes, `cumsum`, static-limit `ffill`, and `roll_mean` on compositional prefix implementations with split-batch state parity. Leave arbitrary rolling callables, dynamic-limit `ffill`, grouped routing, and model solves sequential unless their full state transition is proven associative.
- Prefer shape-directed kernels when asymptotic and measured crossover points differ (for example, fused pairwise cross-sectional rank below a documented small-width threshold and sort/search above it); preserve one semantic contract across every selected kernel.
...
- Keep batch output disk-backed by default (`run_batch_from_mapping(..., out_path=...)`) to avoid large RAM materialization; use `out_path=None` only when in-memory output is explicitly desired. For aggregate formulas that do not need full intermediate frames, prefer DSL reductions such as NaN-aware `sum(x, axis=[0])`, `prod(x, axis=[0])`, `count(x, axis=[0])`, `mean(x, axis=[0])`, or `std(x, axis=[0])`; reductions including time axis `0` should be pushed to their reduced subgraphs and stream/chunk-accumulate rather than allocating the full `(time, ...)` intermediate.
