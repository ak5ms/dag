...
- Native-plan optimization must remain semantics-safe for stateful nodes: DCE, literal folding, stateless CSE, and alias removal happen before liveness, while stateful/grouped nodes remain unique and ordered. Native compute may release the GIL only after validating arrays and retaining any force-cast owners for the complete call. Native batch DAG scheduling must preserve the timestep boundary for state semantics, schedule only dependency-ready nodes, use disjoint per-node scratch, and keep worker synchronization outside numerical kernels.
