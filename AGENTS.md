...
- Native-plan optimization must remain semantics-safe for stateful nodes: DCE, literal folding, stateless CSE, and alias removal happen before liveness, while stateful/grouped nodes remain unique and ordered. Native compute may release the GIL only after validating arrays and retaining any force-cast owners for the complete call. Native batch DAG scheduling must preserve streaming state semantics and remain operator-generic. Multi-worker coordination uses atomic dependency/epoch state without mutexes, condition variables, or operator-specific batch branches; keep synchronization outside numerical kernels.
