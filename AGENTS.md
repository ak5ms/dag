...
- Performance attribution must preserve the compiled formula graph: use JAX trace scopes plus `JaxFlatRuntime.get_execution_plan()`/`profile_batch(...)` rather than timing separately executed subformulas. Remember that XLA fusion can combine adjacent formula nodes into one kernel.
