...
- Native-plan optimization must remain semantics-safe for stateful nodes: DCE, literal folding, stateless CSE, and alias removal happen before liveness, while stateful/grouped nodes remain unique and ordered. Native compute may release the GIL only after validating arrays and retaining any force-cast owners for the complete call. Native batch DAG scheduling must preserve streaming state semantics, use coarse partitions with disjoint state/output ownership, and keep worker synchronization out of per-timestep numerical kernels; do not restore fine-grained per-node/per-row dispatch.
