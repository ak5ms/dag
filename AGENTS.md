...
- Experimental native C++ JAX-flat tick core: Python lowering/wrapper lives in `src/trading_dsl_engine/jax_flat/engine_cpp.py`; native extension code is split between `src/trading_dsl_engine/jax_flat/engine.cpp` and `src/trading_dsl_engine/jax_flat/ops.cpp` (keep this optional, flattened, and allocation-conscious; batch helpers must reuse the same non-batch row transition, unsupported formulas should fail clearly rather than silently falling back, and `TRADING_DSL_ENGINE_DISABLE_CPP_ACCEL=1` must force the pure JAX path for behavior checks).
