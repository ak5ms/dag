...
- Native DAG workers are runtime-owned: `workers=None` resolves to all logical CPUs and `workers=1` is strictly serial. Preserve the immutable level schedule, caller-thread small/narrow fast path, per-timestep completion barrier, executor isolation/lifetime, and single-threaded Eigen-inside-DAG policy. Full-native and hybrid islands must propagate the same worker configuration; do not add per-call batch overrides.
