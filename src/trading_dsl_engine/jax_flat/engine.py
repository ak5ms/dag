...
class JaxFlatRuntime(eqx.Module):
    ...
    def _run_batch_once(self, inputs, states=None, out_path: str | bool = False):
        ...
        if self.cpp and not self.program.cache_nodes and not states and not _has_memmap_input(inputs) and not out_path:
            accelerated = _try_cpp_accelerated_batch(self, inputs, states, emit_warning=False)
            if accelerated is not None:
                return accelerated
        if self.cpp and not self.program.cache_nodes and not states and not _has_memmap_input(inputs) and not out_path:
            hybrid = _try_cpp_hybrid_batch(self, inputs)
            if hybrid is not None:
                return hybrid
            _try_cpp_accelerated_batch(self, inputs, states, emit_warning=True)
...
def _try_cpp_accelerated_batch(runtime: JaxFlatRuntime, inputs, states=None, *, emit_warning: bool = True):
...
def _try_cpp_hybrid_batch(runtime: JaxFlatRuntime, inputs):
    """Run supported native subgraphs first, then finish the full formula in JAX.

    This intentionally targets coarse stateful/grouped subgraphs. Unsupported
    nodes (for example user-supplied stateless JAX lambdas) remain in the JAX
    program, while their supported C++ children are materialized as extra batch
    inputs. Explicit caller state is not accepted because the hybrid owns native
    subgraph state separately from the residual JAX state.
    """
    if os.getenv("TRADING_DSL_ENGINE_DISABLE_CPP_ACCEL", "0") == "1":
        return None
    ...
    try:
        ...
        _warn_cpp_fallback(runtime, f"C++ jax_flat hybrid accelerator unavailable ({type(exc).__name__}: {exc}); falling back to JAX-flat")
        return None
    ...
    staged = _try_cpp_staged_output_batch(runtime, inputs, tuple(candidate_programs), tuple(extra_inputs))
...
def _try_cpp_staged_output_batch(runtime: JaxFlatRuntime, inputs, upstream_node_ids: tuple[int, ...], upstream_values: tuple[np.ndarray, ...]):
    """Finish a supported root in C++ after computing JAX-only frontier nodes.

    This handles formulas shaped like ``cpp(jax_only(cpp(...)), cpp(...))``: C++
    first materializes supported upstream islands, JAX computes the unsupported
    frontier values, and a final native subprogram consumes those frontier arrays
    plus original inputs to produce the root output.
    """
    ...
    if core is None:
        ...
        _CPP_ACCELERATOR_CACHE[key] = core
    state = core.init_state(n_instruments)
    cpp_inputs = tuple(np.asarray(arr, dtype=np.float64) for arr in inputs) + tuple(np.asarray(arr, dtype=np.float64) for arr in frontier_values)
...
def _is_cpp_boundary_op(op: Op) -> bool:
    return (
        isinstance(op, (InputOp, LiteralOp, CacheOp, GroupByOp, CumsumOp, EwmOp, FFillOp, RollingMeanOp, ShiftOp, RbfBasisOp, FutureRbfBasisSumOp, InstrumentBasisMeanOp, RidgeOp))
...
def _subprogram_for_node_with_frontier(program: StreamingProgram, node_id: int) -> tuple[StreamingProgram, tuple[int, ...]]:
    ...
    from trading_dsl_engine.jax_flat.engine_cpp import _cpp_node_specs
...
def _cpp_hybrid_candidates(program: StreamingProgram) -> tuple[int, ...]:
    selected: list[int] = []
    covered: set[int] = set()
    # Prefer large/coarse subgraphs nearest the root, then avoid nested duplicates.
    for node_id in range(len(program.nodes) - 1, -1, -1):
        ...
        is_supported_stateless = isinstance(op, NaryOp) and op.cpp_name is not None and any(program.nodes[cid].op.is_stateful for cid in _ancestor_ids(program, node_id) if cid != node_id)
