...
public:
    MMapFile(const char* path, bool writable, std::size_t create_size = 0) {
        ...
        if (create_size) {
            ...
            const bool resize=static_cast<std::size_t>(st.st_size)!=create_size;
            if (resize && ::ftruncate(fd_,static_cast<off_t>(create_size))!=0)
                throw std::runtime_error(std::string("ftruncate: ")+std::strerror(errno));
#if defined(__linux__)
            // Extent allocation belongs in setup, not the timed row loop. Do not
            // retruncate an already-correct output: every row is overwritten and
            // retaining its pages makes repeated benchmark/execution runs honest.
            if (resize) (void)::posix_fallocate(fd_,0,static_cast<off_t>(create_size));
#endif
            size_=create_size;
        } else {
            struct stat st{};
        ...
        if (size_==0) throw std::runtime_error("cannot mmap an empty file");
