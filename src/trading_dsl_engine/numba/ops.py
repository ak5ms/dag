...
from trading_dsl_engine.base.registry import REGISTRY, CompiledNode, OpSpec, TypeInfo
...
def _make_local_value_node(type_info: TypeInfo) -> CompiledNode:
    if type_info.kind not in ("scalar", "vector", "matrix"):
        raise ValueError("groupby lhs must emit scalar/vector/matrix, not object")

    spec = [("initialized", boolean), ("out", float64[:, :])]
...
def _groupby_validator(types: list[TypeInfo]) -> TypeInfo:
    ...
    if len(types) == 3 and types[1].kind == "object":
        raise ValueError("groupby lhs arg must emit scalar/vector/matrix, not object")
    op_type = types[-1]
...
def _groupby_builder(children: list[CompiledNode], literals: list[float]) -> CompiledNode:
    ...
    class GroupByOp:
        ...
        def on_data(self, frame2d):
            ...
            self._ensure_scratch(scratch_rows, scratch_cols, n_instruments)
...
def register_builtin_ops() -> None:
    ...
    make_nary_op("lt", 2, lambda args: args[0] < args[1])
    make_nary_op("gt", 2, lambda args: args[0] > args[1])
    make_nary_op("abs", 1, lambda args: np.abs(args[0]))
    make_nary_op("isnan", 1, lambda args: 1.0 if np.isnan(args[0]) else 0.0)
    make_nary_op("fillna", 2, lambda args: args[1] if np.isnan(args[0]) else args[0])
    make_nary_op("ln", 1, lambda args: np.log(args[0]))
    make_nary_op("ceil", 1, lambda args: np.ceil(args[0]))
    make_nary_op("floor", 1, lambda args: np.floor(args[0]))
    make_nary_op("exp", 1, lambda args: np.exp(args[0]))
    make_nary_op("sign", 1, lambda args: np.sign(args[0]))
    make_nary_op("fraction", 1, lambda args: np.modf(args[0]))
    make_nary_op("purify", 1, lambda args: args[0] if np.isfinite(args[0]) else np.nan)
    make_nary_op("arctan", 1, lambda args: np.arctan(args[0]))
    make_nary_op("pow", 2, lambda args: args[0] ** args[1])
