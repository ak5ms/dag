...
def _core(reg, family, args, ret, variant, op=None):
    reg.add(
        family,
        partial(row_pset._core_call, op or family, ret),
        args,
        ret,
        variant=variant,
    )
...
def register_tensor_elementwise(reg, ranks):
    ...
    for rank in sorted(set(ranks)):
        ...
        for type_ in tensor_types_for_rank(rank):
            ...
            for family in ("add", "minimum", "maximum"):
                _core(reg, family, (PositiveNumber, type_), type_, "scalar_" + tag)
            _core(reg, "mod", (PositiveNumber, type_), derived, "scalar_" + tag)

            for family in compare:
                _core(reg, family, (type_, type_), boolean, tag)
                _core(reg, family, (type_, PositiveNumber), boolean, tag + "_scalar")
                _core(reg, family, (PositiveNumber, type_), boolean, "scalar_" + tag)

            _core(reg, "where", (boolean, type_, type_), type_, tag)
            _core(
                reg,
                "where",
                (boolean, type_, PositiveNumber),
                type_,
                tag + "_scalar_false",
            )
            _core(
                reg,
                "where",
                (boolean, PositiveNumber, type_),
                type_,
                tag + "_scalar_true",
            )
            reg.add(
                "clip",
                partial(row_pset._clip_call, type_),
                (type_, type_, type_),
                type_,
                variant=tag,
            )
            reg.add(
                "clip",
                partial(row_pset._clip_call, type_),
                (type_, PositiveNumber, PositiveNumber),
                type_,
                variant=tag + "_scalar",
            )
            ...
            _core(reg, "mul", (type_, PositiveNumber), type_, tag + "_scalar")
            _core(reg, "mul", (PositiveNumber, type_), type_, "scalar_" + tag)
            _core(reg, "div", (type_, PositiveNumber), type_, tag + "_scalar")
            inverse_ret = dim if issubclass(type_, dim) else derived
            _core(reg, "div", (PositiveNumber, type_), inverse_ret, "scalar_" + tag)
            reg.add(
                "floordiv",
                partial(row_pset._floordiv_call, type_),
                (type_, PositiveNumber),
                type_,
                variant=tag + "_scalar",
            )
            reg.add(
                "floordiv",
                partial(row_pset._floordiv_call, inverse_ret),
                (PositiveNumber, type_),
                inverse_ret,
                variant="scalar_" + tag,
            )
            _core(
                ...
                "pow",
                (type_, PositiveNumber),
                dim if issubclass(type_, dim) else derived,
                tag + "_scalar",
            )
            ...
            reg.add(
                "floordiv",
                partial(row_pset._floordiv_call, dim),
                (type_, type_),
                dim,
                variant=tag,
            )
        ...
        for family in ("and", "and_", "or", "or_", "xor"):
            _core(
                ...
                boolean,
                f"tensor_rank_{rank}",
                {"and": "and_", "or": "or_"}.get(family, family),
            )
    ...
    families.update(
        ...
        + unitless
        + (
            "where", "clip", "abs", "purify", "floor", "ceil", "round",
            "isnan", "isfinite", "sqrt", "mul", "div", "floordiv", "pow",
            "and", "and_", "or", "or_", "xor", "logical_not",
        )
    )
