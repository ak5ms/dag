...
class NumericTensor(ExprValue):
    """Base GP value for one streamed row with one or more feature axes."""
...
def tensor_type(rank: int, semantic: TensorSemantic) -> type[NumericTensor]:
    """Return the canonical GP class for ``(rank, semantic)``."""
    ...
    result = type(
        prefix + str(rank),
        (parent,),
        {"tensor_rank": rank, "tensor_semantic": semantic},
    )
    _CLASSES[key] = result
    return result
...
def tensor_types_for_rank(rank: int) -> tuple[type[NumericTensor], ...]:
    """Concrete semantic tensor classes registered as GP inputs."""

    return tuple(
        tensor_type(rank, semantic)
        for semantic in _PARENTS
        if semantic != "numeric"
    )


def reduced_type(
    type_: type[NumericTensor], semantic: TensorSemantic | None = None
):
    """Type after reducing exactly the final feature axis."""
...
class TensorFieldSpec:
    """Tensor terminal, external or composed from existing row columns.

    ``feature_shape`` excludes the instrument axis. Thus an external runtime
    array ``(time, instrument, level, channel)`` has ``feature_shape=(level,
    channel)`` and is a rank-3 GP tensor. Composed tensors are currently
    restricted to one feature axis because ``cat`` builds a matrix from rows.
    """
    ...
    def __post_init__(self) -> None:
        ...
        if (
            not name
            or self.semantic not in _PARENTS
            or self.semantic == "numeric"
            or not shape
        ...
        if columns and (len(shape) != 1 or len(columns) != shape[0]):
            raise ValueError(
                "composed tensor columns must equal its single feature extent"
            )
...
DEFAULT_TENSOR_FIELDS = (
    ...
    TensorFieldSpec(
        ...
        "volume",
        (20,),
        _levels(("volume_a", "volume_b")),
    ),
)


def tensor_input_types(
    fields: tuple[TensorFieldSpec, ...], n_instruments: int
) -> dict[str, InputTypeSpec]:
    return {
        field.name: field.input_type(n_instruments)
        for field in fields
        if field.external
    }
