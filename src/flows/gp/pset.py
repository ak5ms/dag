...
from flows.gp.grammar import GrammarPolicy
...
class GPConfig:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.grammar, GrammarPolicy):
...
class _Registrar:
    def __init__(
        ...
        *,
        policy: GrammarPolicy | None = None,
    ) -> None:
        self.pset = pset
        self.policy = policy or GrammarPolicy()
        self.section = "unclassified"
        self.families: set[str] = set()
        self.primitive_family: dict[str, str] = {}
        self.primitive_section: dict[str, str] = {}
        self._names: set[str] = set()

    def set_section(self, section: str) -> None:
        self.section = section

    def add(self, family: str, fn, args: Sequence[type], ret: type, *, variant: str) -> bool:
        if not self.policy.allows(self.section, family):
            return False
        ...
        self.primitive_section[name] = self.section
        return True
...
def _add_terminals(pset: gp.PrimitiveSetTyped, config: GPConfig) -> dict[str, str]:
    ...
    for field_name, info in metadata.items():
        ...
        if row_type not in {TimestampRow, DurationRow, TradingDayHorizonRow}:
            _add_terminal(
                pset,
                GroupVectorInput(dsl.var(field_name)),
                GroupVectorInput,
                f"group_vector_input_{field_name}",
            )
        if info.integer and math.isfinite(info.lower) and math.isfinite(info.upper):
            lower = math.ceil(info.lower)
            upper = math.floor(info.upper)
            if upper >= lower:
                _add_terminal(
                    pset,
                    GroupKeyInput(
                        key(
                            dsl.var(field_name),
                            num_keys=upper - lower + 1,
                            offset=lower,
                            row_scalar=False,
                        )
                    ),
                    GroupKeyInput,
                    f"group_key_input_{field_name}",
                )
...
def make_pset(config: GPConfig | None = None) -> gp.PrimitiveSetTyped:
    """Build the strongly typed GP grammar selected by ``config.grammar``.

    Every registrar uses the same :class:`GrammarPolicy`; disabling a section
    or family therefore changes registration, metadata, generation, and
    introspection together. The full-coverage assertion is retained for the
    default grammar, where it catches accidental omissions from the canonical
    DSL/cpp_stream inventory.
    """

    config = config or GPConfig()
    pset = gp.PrimitiveSetTyped("alpha", [], NumericRow)
    field_terminal_names = _add_terminals(pset, config)
    reg = _Registrar(pset, policy=config.grammar)

    row_sections = (
        ("row.elementwise", _same_type_ops),
        ("row.elementwise", _scalar_broadcast_ops),
        ("row.elementwise", _binary_numeric_ops),
        ("row.elementwise", _logical_ops),
        ("row.elementwise", _unary_ops),
        ("row.ewm", _ewm_ops),
        ("row.rolling", _rolling_ops),
        ("row.alpha", _alpha_ops),
        ("row.cross_sectional", _cross_sectional_ops),
        ("row.temporal", _time_and_history_ops),
        ("row.regression", _regression_ops),
        ("row.reductions", _reductions),
    )
    for section, register in row_sections:
        reg.set_section(section)
        register(reg)
    ...
    pset.gp_full_operator_families = EXPECTED_GP_OPERATOR_NAMES
    pset.gp_dsl_operator_families = EXPECTED_DSL_OPERATOR_NAMES & reg.families
    pset.gp_composite_operator_families = GP_COMPOSITE_OPERATOR_NAMES & reg.families
    pset.gp_cpp_stream_utility_families = GP_CPP_STREAM_UTILITY_OPERATOR_NAMES & reg.families
    ...
    pset.gp_policy_excluded_families = EXPECTED_GP_OPERATOR_NAMES - reg.families
