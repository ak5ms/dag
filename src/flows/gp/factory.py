...
def _active_ranks(config: GPConfig) -> tuple[int, ...]:
    return tuple(sorted({rank for field in config.tensor_fields for rank in range(2, field.logical_rank + 1)}))
...
def make_pset(config: GPConfig | None = None):
    ...
    expected = BASE_EXPECTED_GP_OPERATOR_NAMES | (TENSOR_VEC_FAMILIES if ranks else frozenset())
    missing = expected - reg.families
    unexpected = reg.families - expected
    if missing or unexpected:
        raise AssertionError(f"tensor GP coverage mismatch: missing={sorted(missing)}, unexpected={sorted(unexpected)}")
